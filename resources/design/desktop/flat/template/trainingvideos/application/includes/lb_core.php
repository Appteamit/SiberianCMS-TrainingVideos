...
class LicenseBoxAPI{
    ...
    public function __construct(){
        $this->product_id = '31C5F3D8';
        $this->api_url = 'https://license.appteam.it/';
        $this->api_key = '7FBC452BA1F0C22626AF';
    ...
    public function verify_license($time_based_check = false, $license = false, $client = false){
        ...
        if($time_based_check && $this->verification_period > 0){
            ...
            $today = date('d-m-Y');
            if(empty($_SESSION["4056f9f6ba656e3"])){
                $_SESSION["4056f9f6ba656e3"] = '00-00-0000';
            }
            if($type == 1){
                $type_text = '1 day';
            }elseif($type == 3){
            ...
            if(strtotime($today) >= strtotime($_SESSION["4056f9f6ba656e3"])){
                $get_data = $this->call_api(
                    'POST',
